package org.example.like.service.aop;
...
public class AuthenticationAspect {
    ...
    @Pointcut("execution(public * org.example.like.service.controller.*.*(..))")
    ...
    public Object authenticationHandler(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
        ...
        if (token != null && token.length() != 0) {
            ...
            if (claims == null) {
                //token解析失败
                throw new RuntimeException("Token: " + token + " Resolving  Failure!");
            }
